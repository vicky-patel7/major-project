...
from modelcc import KeyPointClassifier
from modelcc import PointHistoryClassifier
...
def main():
    ...
    # Label Loading ### ## ##
    with open(
        "modelcc/keypoint_classifier/keypoint_classifier_label.csv", encoding="utf-8-sig"
    ) as f:
        keypoint_classifier_labels = csv.reader(f)
        keypoint_classifier_labels = [row[0]
                                      for row in keypoint_classifier_labels]
    with open(
        "modelcc/point_history_classifier/point_history_classifier_label.csv",
        encoding="utf-8-sig",
    ) as f:
    ...
    while True:
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(
                ...
                try:
                    if (
                        handedness.classification[0].label[0:] == "Right"
                        and hand_sign_id == 14
                    ):
                        activeVolume = True
                    else:
                ...
                mouse_x = int(
                    display_size.width * ((cx - area_x1) / (area_x2 - area_x1))
                )
                mouse_y = int(
                    display_size.height *
                    ((cy - area_y1) / (area_y2 - area_y1))
                )
                mouse_x = 0 if mouse_x < 0 else mouse_x
                mouse_y = 0 if mouse_y < 0 else mouse_y
                mouse_x = (
                    display_size.width if mouse_x > display_size.width else mouse_x
                )
                mouse_y = (
                    display_size.height if mouse_y > display_size.height else mouse_y
                )
                print(hand_sign_id)
                if hand_sign_id == 0:
                    Controls.cut()
                ...
                if hand_sign_id == 5 or prev_hand_sign_id == 11:
